...
- 전화번호 기준 중복 입장 방지와 현재 입장 중에만 사용하는 영문 대문자 2글자 임시 ID 발급
- 퇴장 즉시 임시 ID를 기록에서 제거하고 다른 방문자에게 재배정
...
- 이름, 현재 입장 중인 임시 ID, 전화번호 뒤 4자리 검색
...
- 두 글자 ID는 현재 입장 중인 방문자에게만 연결되며 퇴장 후 기록·CSV·감사 로그에 보존되지 않습니다.
