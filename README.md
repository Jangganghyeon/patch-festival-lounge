...
- 입장 완료 ID를 10초간 표시한 뒤 다음 방문자 등록 화면으로 자동 전환
- 독립 퇴장 화면: 이름·전화번호·참가자 ID가 모두 일치할 때만 처리하며 화면 내부는 붉은 디자인으로 구분
- 이름·전화번호 기준 동일인 확인과 1인당 최대 5회 입장 제한
- 한 사람에게 계속 유지되고 다른 사람에게 재배정되지 않는 영문 대문자 고유 ID 발급
- 최초 676명은 2글자, 677번째 고유 방문자부터는 3글자 ID 발급
...
| 퇴구 노트북/태블릿 | 이름·전화번호·고유 ID 퇴장 처리 | `?view=checkout` |
...
- 2~3글자 ID 입력과 동일인 재입장 시 기존 ID 유지 여부 확인
- 5번째 입장까지 허용되고 6번째 등록이 차단되는지 확인
...
- 고유 ID는 동일인에게 계속 유지되며 퇴장·화면 초기화 이후에도 다른 사람에게 재배정되지 않습니다.
- 이름과 전화번호를 함께 정규화한 HMAC 식별값으로 동일인을 확인하며 전화번호 원문은 계속 암호화해 저장합니다.
