...
DEFAULT_START_POINTS = "25000"
START_POINTS_DEFAULT_VERSION = "2"

DEFAULT_SETTINGS = {
    "event_name": "PATCH FESTIVAL LOUNGE",
    "event_subtitle": "Software Club Festival · Live Operations",
    "general_start_points": DEFAULT_START_POINTS,
    "vip_start_points": DEFAULT_START_POINTS,
    "leaderboard_size": "10",
    "privacy_retention_days": "30",
    "public_display_reset_at": "",
    "start_points_default_version": START_POINTS_DEFAULT_VERSION,
}


def _upgrade_start_point_defaults(session) -> None:
    """Raise legacy start-point settings to the current default exactly once."""
    if session.get(AppSetting, "start_points_default_version") is not None:
        return
    if session.scalar(select(AppSetting).limit(1)) is None:
        return
    for key in ("general_start_points", "vip_start_points"):
        row = session.get(AppSetting, key)
        if row is None:
            session.add(AppSetting(key=key, value=DEFAULT_START_POINTS))
        else:
            row.value = DEFAULT_START_POINTS
...
def create_db(config: RuntimeConfig | None = None) -> tuple[Engine, sessionmaker]:
    ...
    with factory.begin() as session:
        _upgrade_start_point_defaults(session)
